...
const NewRecommendations: React.FC<NewRecommendationsProps> = ({ apps, onSelect }) => {
  const newApps = apps.filter((a) => a.status === '新品');
  ...
  return (
    <div className="new-recs">
      <div className="new-recs__header">
        ...
        <span className="new-recs__count">{newApps.length} 款新品</span>
        <span className="new-recs__hint">横向滑动查看 →</span>
      </div>
      <div className="new-recs__list">
        {newApps.map((app) => (
          <div key={app.id} className="new-rec-card" onClick={() => onSelect(app)}>
            ...
            <span className="new-rec-card__date">{app.publishDate}</span>
          </div>
        ))}
      </div>
    </div>
