...
生成 worker 是可恢复的 at-least-once 执行：模型输出返回后立即持久化为 `staged_html`，重启时从暂存内容继续，因此同一请求最多创建一个版本。模型响应与本地 SQLite 暂存无法跨服务原子提交；若进程恰在该窗口退出，可能再次调用模型。严格的模型调用 exactly-once 需要模型提供方提供幂等请求或可恢复流。
...
交付通知采用 at-least-once 语义和稳定的 `X-Idempotency-Key`。`interface-agent` 在成功响应后持久化 `notified_at` 并在恢复时跳过已记录通知；由于外部 HTTP 接收与本地 SQLite 写入无法原子提交，下游流水线必须按幂等键持久去重，才能形成端到端 exactly-once 推进。
