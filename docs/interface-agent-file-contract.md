...
```bash
curl -s -H "$H" -X POST "$OS/api/v1/files/mkdir" \
  -H "Content-Type: application/json" \
  -d '{"path":"共享/interface-agent"}'

curl -s -H "$H" -X POST "$OS/api/v1/files/upload?path=共享/interface-agent" \
...
```bash
curl -s -H "$H" \
  "$OS/api/v1/files/download?path=共享/interface-agent/prototype.html" \
  -o prototype.html
```
...
```bash
curl -s -H "$H" \
  "$OS/api/v1/files/stat?path=共享/interface-agent/prototype.html"
```
...
```text
共享/interface-agent/tasks/<taskId>/pending.md
共享/interface-agent/tasks/<taskId>/prototype.html
共享/interface-agent/tasks/<taskId>/manifest.json
```
