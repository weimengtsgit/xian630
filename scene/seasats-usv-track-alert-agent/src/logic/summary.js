...
export function buildSummary(analysis = {}, params = {}) {
  ...
  const abnormalTargets = targets.filter((t) => t.status === "异常行为舰艇");
  ...
  if (abnormalTargets.length === 0 && threatLevel === "none") advice.push({ level: "low", text: `当前无舰艇接近国土警戒区` });

  const tracked = targets.filter((t) => t.hasObservedTrack);
  let narrative;
  if (tracked.length === 0) {
    narrative = `当前 ${targets.length} 艘舰艇仅有最新位置，无连续轨迹，暂无明显异常活动。`;
  } else {
