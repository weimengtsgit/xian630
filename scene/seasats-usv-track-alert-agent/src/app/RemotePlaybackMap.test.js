...
test("does not render selected vessel name, MMSI, or playback time source", () => {
  const markup = render();

  assert.doesNotMatch(markup, /舰艇：/);
  assert.doesNotMatch(markup, /海巡 630/);
  assert.doesNotMatch(markup, /413000630/);
  assert.doesNotMatch(markup, /2026-07-02 08:00 至 10:00/);
  assert.doesNotMatch(markup, /时间来源/);
});
