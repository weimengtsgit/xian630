...
test("getVesselOverride returns null for empty placeholder entries and resolves filled ones", () => {
  for (const [mmsi, entry] of Object.entries(VESSEL_NAME_OVERRIDES)) {
    const isEmpty = !(String(entry.name || "").trim() || String(entry.code || "").trim());
    const override = getVesselOverride(mmsi);
    if (isEmpty) {
      assert.equal(override, null, `占位条目 ${mmsi} 不应生效`);
    } else {
      assert.notEqual(override, null, `已填充条目 ${mmsi} 应生效`);
    }
  }
});
