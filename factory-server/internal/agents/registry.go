// Package agents holds the fixed registry of factory agents: the Claude Code
// subagents that make up the collaboration-driven generate-test-deploy
// pipeline. The registry is upserted into the store on every server startup so
// its rows always reflect the current design even as their enabled flag is
// toggled at runtime.
package agents

import "github.com/weimengtsgit/xian630/factory-server/internal/model"

// DefaultRegistry returns the fixed factory agents in collaboration-plan order
// (sort_order ascending). The id / key / claude_agent_name / sort_order values
// are stable across releases; name/role/description are sourced from the
// collaboration plan. Every agent ships enabled and belongs to the
// software_development category. Security reviewer is seeded even though it is
// conditional in the default plan so the right-side agent registry can show the
// full set of available collaborators.
//
// The historical combined build-deploy agent (Key "deployer", Role
// "deployment") is split into two: image-builder (image_build) and deployer
// (deployment). Historical job_steps rows that recorded agent_key="deployer"
// for an image_build step still resolve via GetAgent("deployer") (now
// role=deployment), so legacy replay is unaffected.
func DefaultRegistry() []model.Agent {
	sd := model.AgentCategorySoftwareDevelopment
	return []model.Agent{
		{
			ID:              "agent_collaboration_orchestrator",
			Key:             "collaboration-orchestrator",
			Name:            "协作编排",
			Role:            "collaboration_orchestration",
			Description:     "生成默认协作计划、解释选择依据、记录用户调整",
			ClaudeAgentName: "collaboration-orchestrator",
			SkillsJSON:      "[]",
			Category:        sd,
			Prompt:          "",
			Enabled:         true,
			SortOrder:       1,
		},
		{
			ID:              "agent_requirement_analyst",
			Key:             "requirement-analyst",
			Name:            "需求分析",
			Role:            "requirement_analysis",
			Description:     "把用户自然语言需求整理为生成应用的明确需求",
			ClaudeAgentName: "requirement-analyst",
			SkillsJSON:      "[]",
			Category:        sd,
			Prompt:          "",
			Enabled:         true,
			SortOrder:       2,
		},
		{
			ID:              "agent_domain_analyst",
			Key:             "domain-analyst",
			Name:            "领域分析",
			Role:            "domain_analysis",
			Description:     "注入领域知识和客户判断口径",
			ClaudeAgentName: "domain-analyst",
			SkillsJSON:      "[]",
			Category:        sd,
			Prompt:          "",
			Enabled:         true,
			SortOrder:       3,
		},
		{
			ID:              "agent_designer",
			Key:             "designer",
			Name:            "设计",
			Role:            "design_contract",
			Description:     "产出视图、布局、组件、交互状态、数据绑定和响应式约束",
			ClaudeAgentName: "designer",
			SkillsJSON:      `["defense-operations-ui","command-dashboard"]`,
			Category:        sd,
			Prompt:          "",
			Enabled:         true,
			SortOrder:       4,
		},
		{
			ID:              "agent_data_integration",
			Key:             "data-integration",
			Name:            "数据接入",
			Role:            "data_integration",
			Description:     "产出真实数据接入计划和演示数据契约",
			ClaudeAgentName: "data-integration",
			SkillsJSON:      "[]",
			Category:        sd,
			Prompt:          "",
			Enabled:         true,
			SortOrder:       5,
		},
		{
			ID:              "agent_solution_designer",
			Key:             "solution-designer",
			Name:            "方案设计",
			Role:            "solution_design",
			Description:     "把需求变成可执行的前端项目设计和文件计划",
			ClaudeAgentName: "solution-designer",
			SkillsJSON:      "[]",
			Category:        sd,
			Prompt:          "",
			Enabled:         true,
			SortOrder:       6,
		},
		{
			ID:              "agent_code_generator",
			Key:             "code-generator",
			Name:            "代码生成",
			Role:            "code_generation",
			Description:     "写入生成应用项目代码并生成 manifest",
			ClaudeAgentName: "code-generator",
			SkillsJSON:      `["software-factory-app"]`,
			Category:        sd,
			Prompt:          "",
			Enabled:         true,
			SortOrder:       7,
		},
		{
			ID:              "agent_code_reviewer",
			Key:             "code-reviewer",
			Name:            "代码审查",
			Role:            "code_review",
			Description:     "阻断明确影响正确性、可部署性、数据诚实、安全或用户行为的问题",
			ClaudeAgentName: "code-reviewer",
			SkillsJSON:      "[]",
			Category:        sd,
			Prompt:          "",
			Enabled:         true,
			SortOrder:       8,
		},
		{
			ID:              "agent_security_reviewer",
			Key:             "security-reviewer",
			Name:            "安全审查",
			Role:            "security_review",
			Description:     "检查公网数据、认证、上传、外部接口、敏感数据、权限和暴露部署面",
			ClaudeAgentName: "security-reviewer",
			SkillsJSON:      "[]",
			Category:        sd,
			Prompt:          "",
			Enabled:         true,
			SortOrder:       9,
		},
		{
			ID:              "agent_tester",
			Key:             "tester",
			Name:            "测试验证",
			Role:            "test_verification",
			Description:     "分析构建日志并生成诊断摘要",
			ClaudeAgentName: "tester",
			SkillsJSON:      "[]",
			Category:        sd,
			Prompt:          "",
			Enabled:         true,
			SortOrder:       10,
		},
		{
			ID:              "agent_product_acceptance",
			Key:             "product-acceptance",
			Name:            "产品验收",
			Role:            "product_acceptance",
			Description:     "对照确认需求摘要、设计契约、数据契约和主要流程验收",
			ClaudeAgentName: "product-acceptance",
			SkillsJSON:      "[]",
			Category:        sd,
			Prompt:          "",
			Enabled:         true,
			SortOrder:       11,
		},
		{
			ID:              "agent_image_builder",
			Key:             "image-builder",
			Name:            "镜像构建",
			Role:            "image_build",
			Description:     "构建应用容器镜像",
			ClaudeAgentName: "image-builder",
			SkillsJSON:      "[]",
			Category:        sd,
			Prompt:          "",
			Enabled:         true,
			SortOrder:       12,
		},
		{
			ID:              "agent_deployer",
			Key:             "deployer",
			Name:            "部署",
			Role:            "deployment",
			Description:     "容器部署与运行时管理",
			ClaudeAgentName: "deployer",
			SkillsJSON:      "[]",
			Category:        sd,
			Prompt:          "",
			Enabled:         true,
			SortOrder:       13,
		},
	}
}
