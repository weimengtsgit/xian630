...
func skillsPromptBlock(skillPaths, blueprintPaths []string, dataPolicy string) string {
	...
	if realData {
		...
		b.WriteString("真实取数失败时（源不可达、覆盖范围不支持、鉴权缺失），应用必须渲染**降级态（Degraded State）**而不是一行裸「数据异常」——这是「所有真实源均失败」时**完整、合规、可交付**的终态，不要为此重试去编造数据，也不要卡死。降级态必须包含：(1) 顶部说明 banner：数据源不可用 + 失败原因 + 已尝试的数据源列表 + 手动重试按钮；(2) 数据视图的结构预览（图表轴标签 / 表格列头 / 卡片标题等「数据回来后会展示什么」的骨架，**严禁填充任何编造数值**，用空数组 / 占位线 / 「—」即可）；(3) 官方数据源链接；(4) 一句「数据恢复后此处将显示…」的说明。降级态组件命名为 EmptyState / DegradedState / DataUnavailable，**不要**使用 mock / fake / dummy / placeholder / sampleData / demoData 等命名（受诚实数据审计约束）。取数必须在**运行时（浏览器端）**进行，`npm run build` 必须能**完全离线**通过，禁止任何构建期取数依赖，保证页面在任何数据状况下都能产出来。把失败原因与已尝试源记入 output.json 的 warnings——交付假数据（含为「让构建通过」而硬编码的看似真实数值）等同于本次生成失败。")
