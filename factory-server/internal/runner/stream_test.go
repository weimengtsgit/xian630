...
// TestClaudeStreamThinkingDeltaGoesOnlyToTaskThinkingEmitter verifies that
// thinking_delta events are routed to the TaskThinkingEmitter and never appear
// in records or traces.
func TestClaudeStreamThinkingDeltaGoesOnlyToTaskThinkingEmitter(t *testing.T) {
	...
	for _, tr := range records.traces {
		if strings.Contains(tr.payload, "private reasoning") {
