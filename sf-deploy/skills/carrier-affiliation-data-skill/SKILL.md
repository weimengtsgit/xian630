...
**4,834,856 AIS rows** (verified live 2026-06-26), `mmsi` populated, ship names
present (e.g. "普林斯顿", "钟云号", "丹尼尔·井上号"). This is the AIS source for
CSG escort ships (DDG/CG/supply) AND for any military-vessel track request — see
the "Military vessel AIS tracks" section for the full contract. Aircraft remain
on establishment-based affiliation (Mode B) until `icao` is populated.
...
**Endpoint, headers, response shape, and the nginx `/ontology/` CORS proxy are
