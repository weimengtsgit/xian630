...
assert.match(workbenchJsx, /requestAbandonRequirement/, 'ConversationWorkbench must route abandon through an in-app confirmation flow')
assert.doesNotMatch(workbenchJsx, /window\.confirm/, 'abandon confirmation must use the in-app dialog, not the browser confirm')
assert.match(workbenchJsx, /cw-more-menu[\s\S]*放弃本次需求/, 'the low-frequency abandon action should live in the header more menu')
assert.doesNotMatch(workbenchJsx, /className="cw-abandon-requirement"/, 'the abandon action should not occupy the composer area')
assert.match(workbenchJsx, /className="cw-confirm-dialog"[\s\S]*放弃本次需求？[\s\S]*确认放弃/, 'abandoning a requirement must use an in-app confirmation dialog')
assert.match(workbenchCss, /\.cw-more-menu/, 'the header more menu must be styled')
assert.match(workbenchCss, /\.cw-confirm-layer[\s\S]*backdrop-filter:\s*blur/, 'the in-app confirm dialog should use a lightweight modal layer')
...
assert.match(workbenchJsx, /TASK_THINKING_FOLLOW_BOTTOM_THRESHOLD/, 'task thinking auto-follow must use a bottom threshold')
assert.match(workbenchJsx, /WORKBENCH_BODY_FOLLOW_BOTTOM_THRESHOLD/, 'workbench body auto-follow must use a bottom threshold')
assert.match(workbenchJsx, /cwBodyScrollRef/, 'workbench body must keep a scroll container ref')
assert.match(workbenchJsx, /cwBodyShouldFollowRef/, 'workbench body must remember whether user is still at the bottom')
assert.match(workbenchJsx, /timelineFollowSignature/, 'workbench body auto-follow must react to streaming timeline content changes')
assert.match(workbenchJsx, /onScroll=\{updateWorkbenchBodyFollowState\}/, 'workbench body must update follow state when the user scrolls')
assert.match(workbenchJsx, /liveThinkingScrollRef/, 'live thinking stream must keep a scroll container ref')
assert.match(workbenchJsx, /liveThinkingShouldFollowRef/, 'live thinking stream must remember whether user is still at the bottom')
assert.match(workbenchJsx, /onScroll=\{updateLiveThinkingFollowState\}/, 'live thinking stream must update follow state when the user scrolls')
assert.match(workbenchJsx, /taskThinkingScrollRef/, 'task thinking block must keep a scroll container ref')
assert.match(workbenchJsx, /taskThinkingShouldFollowRef/, 'task thinking block must remember whether user is still at the bottom')
assert.match(workbenchJsx, /scrollHeight - scrollTop - clientHeight/, 'task thinking block must detect whether the user is at the bottom')
assert.match(workbenchJsx, /scrollTop = scrollHeight/, 'task thinking block must only auto-scroll when following is enabled')
assert.match(workbenchJsx, /onScroll=\{updateTaskThinkingFollowState\}/, 'task thinking block must update follow state when the user scrolls')
assert.match(workbenchCss, /\.cw-task-thinking-scroll/, 'task thinking scroll region must have dedicated styles')
assert.match(workbenchCss, /\.cw-task-thinking-scroll\s*\{[\s\S]*overflow-y:\s*auto/, 'task thinking scroll region must scroll independently')
