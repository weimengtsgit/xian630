...
assert.match(graphComponent, /visibleCards\s*=\s*isRevealMode[\s\S]*wave\.cards\.filter\(card => revealedKeys\.has\(card\.agentKey\)\)/, 'unconfirmed graph should remove unrevealed cards from wave layout instead of reserving empty card slots')
...
assert.match(graphComponent, /isRevealMode && visibleCards\.length === 0\) return null/, 'unconfirmed graph should not render empty future waves before they are revealed')
assert.match(graphComponent, /!\s*isRevealMode \|\| visibleEdges\.length > 0/, 'unconfirmed graph should not render fallback connector lines when no reveal edge is visible')
assert.match(graphComponent, /ceg-is-orchestrating/, 'graph component should have orchestrating state class')
assert.match(graphComponent, /graph\.confirmed/, 'graph component should check confirmed state to bypass reveal')
