...
assert.match(graphComponent, /ceg-edge-path/, 'graph component should render one visual path per dependency edge instead of merging all edges into one state')
assert.match(graphComponent, /orthogonalPath/, 'graph component should route wave dependencies as fork/merge orthogonal paths')
...
assert.match(graphCss, /@keyframes cegFlowDash/, 'graph css should define animated flow lines')
